...
//   - 23505 (UNIQUE org_id, name) maps to 409 reason="name_collision"
//     (NOT external_id_collision — no external_id column).
package catalog
...
func (h *Handlers) CreateBreakReason(ctx context.Context, req api.CreateBreakReasonRequestObject) (api.CreateBreakReasonResponseObject, error) {
	...
	if err != nil {
		// No CreateBreakReason409JSONResponse is generated by oapi-codegen
		// for this op — the spec does not declare a 409 response. A 23505
		// UNIQUE(org_id, name) collision therefore surfaces as 500 with
		// the canonical name_collision reason so clients can still branch
		// (D-36: branch on error code, not status). The acceptance test
		// pins the reason to "name_collision" regardless of status.
		status, code, reason := mapPgError(err, "break_reason")
		if status == 409 {
			return api.CreateBreakReason500JSONResponse{InternalServerErrorJSONResponse: api.InternalServerErrorJSONResponse{
				Error: code, Reason: "name_collision",
			}}, nil
		}
...
func (h *Handlers) UpdateBreakReason(ctx context.Context, req api.UpdateBreakReasonRequestObject) (api.UpdateBreakReasonResponseObject, error) {
	...
	if err != nil {
		h.deps.Logger.ErrorContext(ctx, "update break_reason begin tx", "err", err)
		return api.UpdateBreakReason500JSONResponse{InternalServerErrorJSONResponse: api.InternalServerErrorJSONResponse{
	...
	// DisplayOrder is `*int` in the request body but `*int32` in the
	// sqlc params — pre-convert to keep the call site narrow.
	...
	row, err := qtx.UpdateBreakReason(ctx, generated.UpdateBreakReasonParams{
		ID:              pgUUID(brID),
		OrgID:           pgUUID(orgID),
	...
	if errors.Is(err, pgx.ErrNoRows) {
		cur, perr := qtx.GetBreakReasonByIdAnyVersion(ctx, generated.GetBreakReasonByIdAnyVersionParams{
			ID:    pgUUID(brID),
			OrgID: pgUUID(orgID),
		})
		...
		// D-56 — 409 path DEL so a stale cached value can't mask the conflict.
	...
	if err := tx.Commit(ctx); err != nil {
		h.deps.Logger.ErrorContext(ctx, "update break_reason commit", "err", err)
		return api.UpdateBreakReason500JSONResponse{InternalServerErrorJSONResponse: api.InternalServerErrorJSONResponse{
...
// mapBreakReason — sqlc-row → api.BreakReason DTO. No nullable fields
// other than CreatedAt/UpdatedAt.
