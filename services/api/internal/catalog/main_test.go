// main_test.go — package-level TestMain for the catalog suite (Plan 03-06).
//
// Brings up ONE Postgres testcontainer + applies all migrations once per
// `go test ./internal/catalog/...` invocation; per-test bring-up reuses
// `sharedPool` (declared in testutil_test.go) for speed (D-73).
//
// Pattern adapted verbatim from services/api/test/isolation/main_test.go.
// The only differences:
//   - Database name: `catalog_test` (vs `isolation_test`) for log grep clarity.
//   - Migrations path: `file://../../../../migrations` from
//     services/api/internal/catalog/  (4 levels up to repo root) — same
//     depth as the isolation suite, but kept literal for documentation.
...
//
// -short skip + CI exit-code policy mirror the isolation suite (D-77 +
...
// Mirrors services/api/test/isolation/main_test.go.
...
// runs every test in the catalog package against it via sharedPool. The
...
func TestMain(m *testing.M) {
	...
	if err != nil {
		os.Stderr.WriteString("catalog: testcontainer postgres unavailable: " + err.Error() + "\n")
		os.Exit(containerFailureExitCode())
	}

	connStr, err := pgC.ConnectionString(ctx, "sslmode=disable")
	if err != nil {
		os.Stderr.WriteString("catalog: connection string: " + err.Error() + "\n")
		_ = pgC.Terminate(ctx)
		os.Exit(containerFailureExitCode())
	}
	...
	if err != nil {
		os.Stderr.WriteString("catalog: sql.Open: " + err.Error() + "\n")
		_ = pgC.Terminate(ctx)
		os.Exit(containerFailureExitCode())
	}
	driver, err := pgmigrate.WithInstance(sqlDB, &pgmigrate.Config{})
	if err != nil {
		os.Stderr.WriteString("catalog: pgmigrate driver: " + err.Error() + "\n")
		_ = sqlDB.Close()
		_ = pgC.Terminate(ctx)
		os.Exit(containerFailureExitCode())
	}
	...
	if err != nil {
		os.Stderr.WriteString("catalog: migrate.NewWithDatabaseInstance: " + err.Error() + "\n")
		_ = sqlDB.Close()
		_ = pgC.Terminate(ctx)
		os.Exit(containerFailureExitCode())
	}
	...
	if err != nil {
		os.Stderr.WriteString("catalog: pgxpool.New: " + err.Error() + "\n")
		_ = pgC.Terminate(ctx)
		os.Exit(containerFailureExitCode())
	}
