...
const finaliseImportJob = `-- name: FinaliseImportJob :one
UPDATE import_jobs
SET status = $1,
    succeeded_rows = $2,
    failed_rows = $3,
...
type FinaliseImportJobParams struct {
...
func (q *Queries) FinaliseImportJob(ctx context.Context, arg FinaliseImportJobParams) (ImportJob, error) {
	row := q.db.QueryRow(ctx, finaliseImportJob,
		arg.Status,
		arg.SucceededRows,
		arg.FailedRows,
		arg.Errors,
...
func (q *Queries) GetImportJob(ctx context.Context, arg GetImportJobParams) (ImportJob, error) {
	row := q.db.QueryRow(ctx, getImportJob, arg.ID, arg.OrgID)
	var i ImportJob
	err := row.Scan(
		&i.ID,
		&i.OrgID,
...
VALUES ($1, $2, $3, 'pending', $4, 0, 0, NULL, $5)
RETURNING id, org_id, entity_type, status, total_rows, succeeded_rows,
          failed_rows, errors, idempotency_key, created_at, updated_at
`
...
const lookupImportJobByIdempotencyKey = `-- name: LookupImportJobByIdempotencyKey :one
SELECT id, org_id, entity_type, status, total_rows, succeeded_rows,
       failed_rows, errors, idempotency_key, created_at, updated_at
FROM import_jobs
...
type LookupImportJobByIdempotencyKeyParams struct {
	OrgID          pgtype.UUID `json:"org_id"`
	IdempotencyKey *string     `json:"idempotency_key"`
}
...
func (q *Queries) LookupImportJobByIdempotencyKey(ctx context.Context, arg LookupImportJobByIdempotencyKeyParams) (ImportJob, error) {
	row := q.db.QueryRow(ctx, lookupImportJobByIdempotencyKey, arg.OrgID, arg.IdempotencyKey)
	var i ImportJob
	err := row.Scan(
		&i.ID,
		&i.OrgID,
...
const sweepCrashedImportJobs = `-- name: SweepCrashedImportJobs :many
UPDATE import_jobs
SET status = 'failed',
...
type SweepCrashedImportJobsRow struct {
	ID    pgtype.UUID `json:"id"`
	OrgID pgtype.UUID `json:"org_id"`
}
...
// concurrent imports.
func (q *Queries) SweepCrashedImportJobs(ctx context.Context) ([]SweepCrashedImportJobsRow, error) {
	rows, err := q.db.Query(ctx, sweepCrashedImportJobs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []SweepCrashedImportJobsRow{}
	for rows.Next() {
		var i SweepCrashedImportJobsRow
