...
Open Routing SHALL evolve toward visual flow authoring, deterministic simulation, publish governance, and runtime execution after the v0.1 catalog foundation is stable.

#### Scenario: Publishing a flow in a later milestone

- **WHEN** a product team publishes a routing flow
- **THEN** the visual graph is the source of truth
- **AND** publish validates graph structure, DSL/export consistency, runtime plan shape, catalog references, and adapter capability references before activation
