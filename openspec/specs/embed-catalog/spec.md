# Embed Catalog Specification

## Purpose

This specification captures the shipped v0.1 Web Component embed bundle for mounting the catalog configuration surface inside host applications.
...
### Requirement: Tarball-ready package contract

The embed package SHALL be packable for v0.1 distribution without requiring npm publication.

#### Scenario: Packing the embed package

- **WHEN** `@open-routing/catalog-embed` is built and packed
- **THEN** the tarball contains the embed JavaScript, lazy chunks, README, license, changelog, and TypeScript declarations
- **AND** package exports resolve for the main embed entry and hash router adapter

### Requirement: Embed bundle budget

The eager embed bundle SHALL stay at or below 70 KiB gzipped.

#### Scenario: Checking bundle size in CI

- **WHEN** CI builds the embed package
- **THEN** the eager gzipped bundle size is measured for `dist/embed.js`
- **AND** the build fails when it exceeds 70 KiB
- **AND** lazy chunks are reported without blocking the budget gate
